...
static PCA9685Servo servoX(pwmDriver, SERVO_X_CHANNEL);
static PCA9685Servo servoY(pwmDriver, SERVO_Y_CHANNEL);

static PIDController pidX(KP, KI, KD, TARGET_X);
static PIDController pidY(KP, KI, KD, TARGET_Y);
static UartPosProvider posProvider(Serial1, CAMERA_BAUD);
